...
```Python\#ways_n_ItemsCanBeArranged
def C(n, N):
...
When doing these calculations it is easy to double count. Always ensure that the N, in N objects does not overlap another N. For example if we have the set of all clubs (N=13), we cannot also have the set of all club honors (N=4). We can have the set of club honors (N=4) and the set of club non-honors (N=9).
